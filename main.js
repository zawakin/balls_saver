...
//レンダリング処理を呼び出す-----------------------------------------------------------------------------------------------
  ...
  //  		return;
	...
	// 	console.log("3:hoge");
		...
		//socket.on("load_data", function(stext){
		//  console.log("got data from server.");
		//  sdatas = JSON.parse(stext);
		//  loaded = true;
...
(function(){
	...
	if(mode>=MODE.SAVE){
		counter_upper_bound = 100;
		if(counter < counter_upper_bound){
			// socket.emit("counter",mouse);
			// alert(leftUp);
			var data = {
			...
			socket.emit("send_data", JSON.stringify(data));
		}
		else if(counter==counter_upper_bound){
			socket.emit("save_end",{});
		}
	}
	if((mode==MODE.LOAD ||　mode==MODE.LOAD_AND_SAVE) && counter <= 50){
		// console.log("counter = " + counter);
		// console.log(typeof(sdatas));
		console.log(sdatas[0]);
		var data = JSON.parse(sdatas[counter-1]);
		keyCode1 = data.k1;
		keyCode2 = data.k2;
		leftUp = data.lU;
		leftDown = data.lD;
